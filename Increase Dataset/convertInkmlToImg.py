...
def parseLG(LG):
    ...
    for lg in LG:
        ...
        lg = lg.replace(" ", "").split(',')
        if lg[0] != "O": continue
        # Select symbol id and associated stroke id (only integers, we remove the letters)
        sym.append([ lg[1][3:], [ l[1:] for l in lg[4:]] ])
    return sym
...
def genHypothesesExample(file):
    ...
    imgs = genImgFromLGHypotheses(traces, LG, 28)
